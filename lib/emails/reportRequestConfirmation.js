...
// No-reply sender. Must be an SES-verified identity: the raceinnovations.in
// DOMAIN is NOT verified, but the raceautoindia.com domain IS — so we send the
// no-reply confirmation from no-reply@raceautoindia.com (which delivers),
// showing "Race Innovations" as the display name. (no-reply@raceinnovations.in
// would be rejected by SES and silently never arrive.)
const FROM_ADDRESS =
...
export async function sendReportRequestConfirmationEmail(data) {
  ...
  try {
    const result = await sendEmail({
      ...
      text,
      from: FROM_ADDRESS,
      // No reply-to: this is a no-reply email; replies go to the unmonitored
      // no-reply mailbox rather than a staffed inbox.
    });
