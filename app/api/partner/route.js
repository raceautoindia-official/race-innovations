...
export async function GET() {
    try {
        const [rows] = await db.query("SELECT * FROM clients_slider");
...
export async function POST(req) {
    try {
        ...
        try {
            const [result] = await db.query("INSERT INTO clients_slider (image_url) VALUES (?)", [imagePath]);
...
export async function PUT(req) {
    try {
        ...
        await db.query("UPDATE clients_slider SET image_url = ? WHERE id = ?", [image_url, id]);
...
export async function DELETE(req) {
    try {
        ...
        await db.query("DELETE FROM clients_slider WHERE id = ?", [id]);
