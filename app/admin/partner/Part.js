...
function Client() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const uploadRes = await fetch("/api/partner", {
        method: "POST",
        body: formData,
      });
      ...
      const updateRes = await fetch("/api/partner", {
        method: "PUT",
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`/api/partner?id=${deleteId}`, {
        method: "DELETE",
      });
          ...
          <div style={modalStyle} onClick={() => setDeleteId(null)}>
            <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", gap: "10px", justifyContent: "center", marginTop: "15px" }}>
                <button style={saveButtonStyle} onClick={confirmDelete}>Yes, Delete</button>
                <button style={deleteButtonStyle} onClick={() => setDeleteId(null)}>Cancel</button>
              </div>
            </div>
          </div>
        )}
...
// Styles (same as before, except cancel/save spacing)
...
const listItemStyle = { textAlign: "center", padding: "10px", color: "black" };
const linkStyle = {
  textDecoration: "none",
  display: "flex",
  alignItems: "center",
